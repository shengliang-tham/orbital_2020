...
class dataBLock():
    ...
    def addtofifteenMin(self,arr):
        ...
        self.fifteenMinArr['MA10'] = self.fifteenMinArr["Close"].ewm(span=10, min_periods=10).mean().apply(lambda x: round(x*1000,2))
        self.fifteenMinArr['MA40'] = self.fifteenMinArr["Close"].ewm(span=40, min_periods=40).mean().apply(lambda x: round(x*1000,2))
...
def startTrading(Symbol):
    def on_message(ws, message):
        if "trade" in message:
            ...
            a.addTimeFrame({'Price': price, 'Time': time[:-3], 'Volume': volume})
