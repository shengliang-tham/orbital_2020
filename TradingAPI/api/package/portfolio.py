...
def getResults(bar):
    ...
    for ticker in tickers:    
        ...
        tabl = soup.find_all("div", {"class": "My(6px) Pos(r) smartphone_Mt(6px)"})
        #print(tabl)
        for t in tabl:
            rows = t.find_all("span", {"class": "Trsdu(0.3s) Trsdu(0.3s) Fw(b) Fz(36px) Mb(-4px) D(b)"})
            # print(rows)
            for row in rows:
               temp_dir["current price"] = row.get_text()
